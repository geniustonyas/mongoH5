...
<script setup lang="ts">
  ...
  const recommendBooks = ref<CommicBookInfo[]>([])
  ...
  const categoryName = computed(() => {
    ...
    return commicCategoryStore.getCategoryNameById(categoryId.toString())
  })
  ...
  const fetchBookDetails = async () => {
    ...
    try {
      ...
      const {
        data: { data }
      } = await getCommicDetail(nid as string)
  ...
  const handleBookClick = (item: CommicBookInfo) => {
    ...
    if (route.query.nid === item.id) {
      ...
      // 不同的书，进行路由跳转
      router.push({ name: 'comicIntro', query })
    }
  }
