export default {
  ...
  overviewDesc: `SEABET.IO는 최고의 암호화폐 기반 베팅 플랫폼입니다. 우리는 라이브 스포츠, 라이브 카지노 및 슬롯 베팅 게임에 관심 있는 플레이어들을 위한 공정하고 안전한 플랫폼을 제공하며, 세계적인 제공업체가 개발한 42만 건 이상의 프리매치 이벤트 및 수천 가지 다른 비디오 슬롯, 게임 테이블 및 라이브 딜러 게임을 제공합니다. SEABET.IO는 기록적인 성장을 이루어왔으며, 당신도 우리의 성공 스토리의 일부가 되실 것을 초대합니다.`,
  bePartner: '파트너가 되기',
  howBePartner: 'SEABET.IO를 홍보하고 할당량 없이 최대 50% 수익을 올리세요! 우리는 세계 각지의 다른 지역 관리자를 대상으로 11개 언어로 서비스를 제공합니다!',
  commissionDetails: '수수료 상세 정보:',
  level: '레벨',
  ...
  retentionA2: '메뉴에서 "등록"을 선택하고 가입하려는 브랜드를 선택한 후 등록을 완료하면 됩니다. 계정이 승인되면 로그인하여 추적 링크를 생성할 수 있습니다.',
  ...
  commissionQ2: '2. 제휴 수익을 정확하게 어떻게 계산할 수 있나요?',
  commissionA2:
    '플레이어가 웹사이트에서 베팅할 때마다, 그 플레이어의 수익의 일정 비율을 받게 됩니다. 당신의 수익 = (게임 내 총 승/패 - 보너스 - 게임 내 총 승/패의 15% 네트워크 수수료 - 입금/출금 수수료) x (당신의 수수료 비율).',
  commissionQ2Desc: '● 네트워크 수수료 = 게임 내 총 승/패의 15%',
  ...
  commissionA5: '제휴 수수료는 매월 첫 5 영업일 이내에 자동으로 지급됩니다. 자금은 당신의 플레이어 계정에 추가되며, 언제든지 웹사이트에서 인출하거나 수수료를 베팅에 사용할 수 있습니다.',
  commissionQ6: '6. 나 자신을 추천할 수 있나요?',
  commissionA6: '아니요, 저희 제휴 프로그램은 이러한 실천을 지지하지 않습니다. 이것은 부정한 행위로 간주될 것입니다.',
  ...
  advantageQ3: '빠른 제휴 지급',
  advantageA3: '빠른 제휴 지급. 우리는 매월 첫 5 영업일 이내에 수수료를 지불합니다.',
  advantageQ4: '주요 브랜드',
