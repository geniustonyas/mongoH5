...
export default {
  ...
  problem: {
    ...
    howComputeInterials: `<p>스포츠 베팅이든 카지노 게임이든, 암호화폐를 사용하면 포인트를 얻을 수 있습니다. 각 베팅으로 얻는 포인트를 계산하기 위해 정산 과정에서 베팅 금액을 실시간 시장 환율로 USDT로 변환합니다. 이렇게 하면 사용된 암호화폐의 종류에 관계없이 모든 포인트 모으기가 공정하게 이루어집니다. 환율 변동으로 인해 포인트 계산 시 가장 가까운 정수로 반올림하는 원칙을 따릅니다.</p>
                          <p>Seabet 포인트를 얻는 공식은 다음과 같습니다: 플레이어의 베팅 금액 * 0.1 * 해당 레벨의 포인트 계수. 예를 들어, 플레이어가 플래티넘 레벨에 있고 5000을 베팅한다면, 포인트 계산 공식에 따라 플레이어는 5000 * 0.01 * 1.3 = 65 포인트를 얻게 됩니다. Seabet 포인트는 다양한 용도로 사용할 수 있으며 플레이어는 포인트 상점에서 동등한 선물과 서비스를 교환할 수 있습니다.</p>`,
